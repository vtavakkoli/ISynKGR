...
def _read_dataset(dataset_dir: Path, max_samples: int) -> list[dict]:
    ...
    opc_files = sorted((dataset_dir.parent / "opcua" / "synthetic").glob("*.xml"))[
        :max_samples
    ]
    return [
        {"id": f.stem, "source_path": str(f), "source_id": f"ns=2;i={1000 + i}"}
        for i, f in enumerate(opc_files)
    ]
...
def _validate_mapping(
    ...
    if not mapping_type:
        ...
        violations.append(
            {
                "type": "mapping_type_invalid",
                "message": f"mapping_type={mapping_type} not in {sorted(ALLOWED_MAPPING_TYPES)}",
            }
        )
    ...
    if target_id and not AAS_ID_PATTERN.match(str(target_id)):
        violations.append(
            {
                "type": "target_id_format",
                "message": f"Invalid target_id format: {target_id}",
            }
        )
    ...
    if confidence is None or float(confidence) < confidence_threshold:
        violations.append(
            {
                "type": "confidence_low",
                "message": f"confidence below threshold ({confidence_threshold}): {confidence}",
            }
        )
        ...
        violations.append(
            {
                ...
                "message": f"Duplicate mapping key: {dedup_key}",
            }
        )
    else:
        seen_keys.add(dedup_key)

    blocking_violations = [v for v in violations if v.get("type") != "confidence_low"]
...
def main() -> None:
    ...
    confidence_threshold = float(
        cfg_data.get("confidence_threshold", os.getenv("CONFIDENCE_THRESHOLD", "0.2"))
    )
    cfg = TranslatorConfig(
        model_name=cfg_data.get("model_name", model_name),
    ...
    for idx, row in enumerate(dataset_rows, start=1):
        ...
        if source_path:
            ...
            sample_path = (
                dataset_dir.parent / "opcua" / "synthetic" / f"opcua_{idx - 1:03d}.xml"
            )
            if not sample_path.exists():
                sample_path = (
                    ...
                    / "opcua"
                    / "synthetic"
                    / f"opcua_{int(str(source_id).split('=')[-1]) - 1000:03d}.xml"
                )
        ...
        llm_error = (
            (result.provenance.metadata or {}).get("llm_error")
            if result.provenance
            else None
        )
        if llm_error:
            llm_bugs.append(
                {"sample": sample_path.name, "mode": mode, "error": llm_error}
            )
        ...
        if result.mappings:
            for m in result.mappings:
                ...
                _, violations = _validate_mapping(
                    record, seen_keys, confidence_threshold
                )
        ...
        valid = len(blocking_item_violations) == 0
        ...
        if idx % 5 == 0 or idx == total:
            ...
            log(
                f"Processed {idx}/{total} ({pct:.1f}%) | avg_time_per_item={avg:.3f}s | ETA={eta:.2f}s"
            )
    ...
    (output_dir / "provenance.json").write_text(
        json.dumps(
            {"mode": mode, "dataset": str(dataset_dir), "llm_bug_count": len(llm_bugs)},
            indent=2,
        )
    )
    ...
    log(
        f"[SUITE-END] total={total} elapsed={_fmt_s(suite_elapsed)} throughput={throughput:.2f}/s bugs={len(llm_bugs)}"
    )
