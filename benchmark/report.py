...
def _bar_svg(
    items: list[tuple[str, float]], title: str, width: int = 800, row_h: int = 28
) -> str:
    if not items:
        return ""
    ...
    for name, value in items:
        ...
        lines.append(
            f'<text x="10" y="{y+14}" font-size="12" font-family="Arial">{safe}</text>'
        )
        lines.append(
            f'<rect x="{left}" y="{y}" width="{bar_w}" height="16" fill="#4e79a7" />'
        )
        lines.append(
            f'<text x="{left + bar_w + 6}" y="{y+13}" font-size="12" font-family="Arial">{value:.3f}</text>'
        )
        y += row_h
    lines.append("</svg>")
    return "\n".join(lines)
...
def write_report(run_dir: Path, rows: list[dict]) -> None:
    ...
    ranked_validity = sorted(
        rows, key=lambda r: r.get("validity_pass_rate", 0.0), reverse=True
    )
        ...
        compact_rows.append(
            {
                ...
                "confidence_low_rate": _fmt(row.get("confidence_low_rate", 0.0)),
                "confidence_low_count": int(row.get("confidence_low_count", 0)),
            }
        )
    ...
    md = [
        ...
        "## Error Taxonomy",
        _markdown_table(
            violation_rows or [{"violation_type": "none", "count": 0}],
    ...
    f1_items = [
        (r.get("baseline", r.get("scenario", "")), float(r.get("f1", 0.0)))
        for r in ranked_f1
    ]
    validity_items = [
        (
            r.get("baseline", r.get("scenario", "")),
            float(r.get("validity_pass_rate", 0.0)),
        )
    ...
    confidence_items = [
        (
            r.get("baseline", r.get("scenario", "")),
            float(r.get("confidence_low_rate", 0.0)),
        )
        for r in rows
    ]
    violation_items = [
        (v["violation_type"], float(v["count"])) for v in violation_rows[:8]
    ]

    summary_table = html.escape(
        _markdown_table(
            compact_rows,
            [
                "baseline",
...
{_bar_svg(confidence_items, "Confidence Low Rate by Baseline")}
...
<pre>{raw_json}</pre>
    ...
    report_json = {
        ...
        "summary": {
            ...
            "confidence_low_count_total": sum(
                int(r.get("confidence_low_count", 0)) for r in rows
            ),
            "confidence_low_rate_avg": (
                sum(float(r.get("confidence_low_rate", 0.0)) for r in rows) / len(rows)
                if rows
                else 0.0
            ),
        },
    }
    (run_dir / "report.json").write_text(json.dumps(report_json, indent=2))
...
def _bar_chart(
    path: Path, names: list[str], values: list[float], title: str, ylabel: str
) -> None:
...
def generate_final_report(results_root: Path = Path("results")) -> Path:
    ...
    _bar_chart(
        charts_dir / "f1_by_scenario.png",
        names,
        [float(r.get("f1", 0.0)) for r in rows],
        "F1 by Scenario",
        "F1",
    )
    _bar_chart(
    ...
    plt.scatter(
        [float(r.get("time_s", 0.0)) for r in rows],
        [float(r.get("f1", 0.0)) for r in rows],
    )
